    ...
    def compress(self, X):
        ...
        for i in range(D.shape[0]):
            for j in range(self.nn + 1):
                G[i,sorted_index[i,j]] = D[i,sorted_index[i,j]]
                G[sorted_index[i,j],i] = D[sorted_index[i,j],i]
        
        distance = utils.dijkstra(G)
        distance[np.isinf(distance)] = distance[~np.isinf(distance)].max()
        ...
        # Solve for the minimizer
        z,f = findMin(self._fun_obj_z, Z.flatten(), 500, distance)
